(function($) {
  	...
  	var GaviasTheme = {
		...
		scrollTop: function(){
	 	...
	 	campaign: function(){
	      ...
	      $(document).on('click', '.love_this_campaign', function () {
	      	if(funden_data.check_login == 'no'){
	      		$.notify(funden_data.str_login,  { className:"waring", position:"right center", autoHideDelay:2000, showDuration:300 });
	      	}
	         ...
	         $.ajax({
	            ...
	            data: {'action': 'love_campaign_action', 'campaign_id': campaign_id, 'security': funden_jquery_object.security_nonce},
	            success:function(data){
	               ...
	               if (data.success == 1){
	                  $(html_wrapper).html('<a href="javascript:;" class="remove_from_love_campaign" data-campaign-id="'+campaign_id+'"><i class="wpneo-icon wpneo-icon-love-full"></i></a>');
	      				$.notify(funden_data.str_add_wishlist,  { className:"waring", position:"right center", autoHideDelay:2000, showDuration:300 });
	               }
	            },
	      ...
	      $(document).on('click', '.remove_from_love_campaign', function () {
	        	...
	        	$.ajax({
	          	type:"POST",
	          	url: wpcf_ajax_object.ajax_url,
	          	data: {'action': 'remove_love_campaign_action', 'campaign_id': campaign_id, 'security': funden_jquery_object.security_nonce},
	          	success:function(data){
	            	data = JSON.parse(data);
	            	$(html_wrapper).html('<a href="javascript:;" class="love_this_campaign" data-campaign-id="'+campaign_id+'"><i class="wpneo-icon wpneo-icon-love-empty"></i></a>');
	      			$.notify(funden_data.str_remove_wishlist,  { className:"waring", position:"right center", autoHideDelay:2000, showDuration:300 });
	          	},
	          	error: function(jqXHR, textStatus, errorThrown){
	            	wpcf_modal({'success':0, 'message':'Error'})
	          	}
	      ...
	      $('.wpneo_login_form_div form#loginform').append('<div>'+funden_data.my_account_link+'</div>');
    	},
		...
		other: function(){
			...
			$('.funden-post-share.style-2 .btn-control-share').on('click', function(e){
				e.preventDefault();
				var wrapper = $(this).parents('.funden-post-share');
	      ...
	      $(document).ajaxComplete(function(event, jqxhr, settings) {
	        ...
	        if (settings.data.indexOf("action=addfrontenddata")!=-1){
	            if(jqxhr.responseText){
	               var data = JSON.parse(jqxhr.responseText);
	               if(data.success){
	                  window.location.href = funden_jquery_object.dashboard_url;
	               }
	            }
	        	}
	    	})
